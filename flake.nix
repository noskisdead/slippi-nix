{
  ...
  outputs =
    ...
    let
      ...
      # Pinned upstream release artifacts (version + hashed src), regenerated by
      # `nvfetcher` from ./nvfetcher.toml. This is the single source of truth for
      # which Slippi versions the flake builds.
      sourcesFor = pkgs: pkgs.callPackage ./_sources/generated.nix { };
    in
    {
      overlays = {
          ...
          testScript =
            ...
            ''
              ...
              with subtest("ensure dolphin appimages report correct versions"):
                  ...
                  '/home/daniel/.config/Slippi Launcher/netplay/Slippi_Online-x86_64.AppImage' --version 2>&1 | grep ${sources.netplay.version}
                  """))
                  machine.succeed(as_user("""
                  '/home/daniel/.config/Slippi Launcher/playback/Slippi_Playback-x86_64.AppImage' --version 2>&1 | grep ${sources.playback.version}
                  """))
            '';
        };
      });
